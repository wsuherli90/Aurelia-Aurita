...
object AureliaVerification {
  def main(args: Array[String]): Unit = {
    SimConfig.withWave.compile(new AureliaAccelerator()).doSim { dut =>
      ...
      dut.io.m_axis_data.ready #= true // Always ready to accept
      
      // Small Grid for TLAST Verification (2x2x2 = 8 Voxels)
      dut.io.grid_dim(0) #= 2
      dut.io.grid_dim(1) #= 2
      dut.io.grid_dim(2) #= 2
      ...
      for(i <- 0 until TOTAL_VOXELS) {
         dut.io.s_axis_data.payload.data #= BigInt(i) * BigInt("10000000000000") // Junk data pattern
         // Valid upstream behavior: TLAST on input is ignored by our core (we count voxels), 
         // but good practice to drive it.
      ...
      // Wait long enough for deep pipeline (Ricci ~10 + Inv ~6 + Axion ~2 + Queues)
      for(_ <- 0 until 100) {
